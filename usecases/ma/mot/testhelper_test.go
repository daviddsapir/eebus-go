...
func TestMaMOTSuite(t *testing.T) {
	suite.Run(t, new(MaMOTSuite))
}

type MaMOTSuite struct {
	suite.Suite
...
func (s *MaMOTSuite) Event(ski string, device spineapi.DeviceRemoteInterface, entity spineapi.EntityRemoteInterface, event api.EventType) {
	s.eventCalled = true
}

func (s *MaMOTSuite) BeforeTest(suiteName, testName string) {
	s.eventCalled = false
	...
	s.sut = NewMOT(localEntity, s.Event)
	s.sut.AddFeatures()
	s.sut.AddUseCase()
