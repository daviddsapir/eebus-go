...
func (e *CRCSF) HandleEvent(payload spineapi.EventPayload) {
...
func (e *CRCSF) updateOperationModes(payload spineapi.EventPayload) {
	if e.coolingSystemFunctionID == nil {
		return
	}
...
func (e *CRCSF) updateSystemFunctionOperationModeRelations(payload spineapi.EventPayload) {
	hvac, err := client.NewHvac(e.LocalEntity, payload.Entity)
	if err != nil {
	...
	if len(relations) != 1 || relations[0].SystemFunctionId != e.coolingSystemFunctionID {
		return
	}

	modeIds := relations[0].OperationModeId
	for _, id := range modeIds {
		// Request the operation mode description by the operation mode ID
	...
	// Request the system functions for the "cooling" system function
	selector := &model.HvacSystemFunctionListDataSelectorsType{
		SystemFunctionId: e.coolingSystemFunctionID,
	}
...
func (e *CRCSF) updateSystemFunctionDescriptions(payload spineapi.EventPayload) {
	...
	// Get the system function ID for the cooling function
	...
	if err != nil || len(systemFunctions) != 1 {
		logging.Log().Errorf("Failed to get cooling function: %v", err)
		return
	}

	// Get the system function ID for the heating function
	e.coolingSystemFunctionID = systemFunctions[0].SystemFunctionId

	// Get the operation mode relations for the cooling function
	...
	if _, err := hvac.RequestHvacSystemFunctionOperationModeRelations(selector, nil); err != nil {
		logging.Log().Errorf("Failed to request HVAC system function setpoint relations: %v", err)
	}
}

func (e *CRCSF) connected(entity spineapi.EntityRemoteInterface) {
	...
	if !hvac.HasSubscription() {
		if _, err := hvac.Subscribe(); err != nil {
			logging.Log().Errorf("Failed to subscribe to HVAC system functions: %v", err)
			return
		}
	}
