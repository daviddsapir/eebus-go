...
func (c *CRCSF) OperationMode(entity spineapi.EntityRemoteInterface) (*usecasesapi.HvacOperationModeType, error) {
	hvac, err := client.NewHvac(c.LocalEntity, entity)
	if err != nil {
		return nil, err
	}

	systemFunction, err := hvac.GetHvacSystemFunctionForId(*c.coolingSystemFunctionID)
	if err != nil {
...
//   - ErrDataNotAvailable: If the operation mode is not (yet) available.
//   - Other: Any other errors encountered during the process.
func (c *CRCSF) WriteOperationMode(
	entity spineapi.EntityRemoteInterface,
	...
	if c.coolingSystemFunctionID == nil {
		return api.ErrDataNotAvailable
	}
	...
	systemFunction.SystemFunctionId = c.coolingSystemFunctionID
