...
func TestCemCDSFSuite(t *testing.T) {
	suite.Run(t, new(CaCDSFSuite))
}

type CaCDSFSuite struct {
	suite.Suite
	...
	cdtEntity        spineapi.EntityRemoteInterface

	eventCalled bool
}

func (s *CaCDSFSuite) Event(
	ski string,
...
func (s *CaCDSFSuite) BeforeTest(suiteName, testName string) {
	s.eventCalled = false
	cert, _ := cert.CreateCertificate("test", "test", "DE", "test")
	configuration, _ := api.NewConfiguration("test", "test", "test", "test",
